...
const main = async () => {
	try {
		...
		if (!characterFilePath) {
			characterFilePath = await promptUser(
				"Please provide the path to the character JSON file:",
				"character.json",
			);
		}

		if (!knowledgeFilePath) {
			knowledgeFilePath = await promptUser(
				"Please provide the path to the knowledge JSON file:",
				"knowledge.json",
			);
		}
		...
		if (!character || !knowledge) {
			console.error(
				"Invalid input files. Please provide valid JSON files for character and knowledge.",
			);
			return;
		}
